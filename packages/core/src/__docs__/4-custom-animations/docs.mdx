...
At its core `element motion` is a powerful orchestration layer which provides a way to collect and trigger animations,
...
> **Tip -** `element motion` works great with [other animation libraries](/advanced-usage#use-with-other-libaries) just incase you're wondering.
...
> **Tip -** Animations are fired from the destination `Animator` element using the origin animations,
...
Composability is a primary feature of `element motion` - with a small change we can compose the two animations together.
...
> **Tip -** Having some trouble using `element motion`?
